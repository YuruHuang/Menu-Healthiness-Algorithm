...
confit_all = data.frame()
for (rest_t in types){
  print(rest_t)
  data_temp = england[type==rest_t]
  model_con = lmer(predicted_score~ imd_decile_cat + urban +(1|lsoa11), data=data_temp)
  confit = confint(model_con)
  confit_row = data.frame(lower = -confit[12,2], upper = -confit[12,1], type = rest_t, 
                          diff = -summary(model_con)$coefficients[10,'Estimate'])
  confit_all = bind_rows(confit_all, confit_row)
...
for (rest_t in types){
  ...
  print('----------------------------')
  data_temp = england[type==rest_t]
  model = lmer(predicted_score~ as.numeric(imd_decile)+urban +(1|lsoa11), data=data_temp)
  print(summary(model))
}
